...
#use constant HISAT    => ('2.0.2-beta'  => '~/hisat2/');
...
if ($user_database_path) {
 ...
 my $hisat2b = "/sonas-hs/ware/hpc_norepl/data/programs/hisat2-2.0.4/hisat2-build";
...
for my $query_file (@file_query) {
   ...
   my $hisat2p = "/sonas-hs/ware/hpc_norepl/data/programs/hisat2-2.0.4/";
    my $app  = $hisat2p.'hisat2';
#    if ($annotation_path) {
#       $HISAT_ARGS .= " -G $annotation_path";
#    }
#    my $align_command = "$app $HISAT_ARGS -x $database_path -1 $query_file -2 $second_file  | samtools view -bS - | samtools sort - $query_file.sorted | samtools index $query_file.sorted.bam";
